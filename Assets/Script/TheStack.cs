...
public class TheStack : MonoBehaviour
{
    ...
    int stackCount = -1;    //미리 1개 쌓아놓을거라 이렇게해야 0부터시작
    ...
    bool Spawn_Block()
    {
        ...
        newTrans.parent = this.transform;   //부모를 바꿔버림
    ...
    bool PlaceBlock()
    {
        ...
        if (isMovingX)
        {
            ...
            if (deltaX > ErrorMargin)   
            {
                ...
                Vector3 tempPosition = lastBlock.localPosition; //임시 포지션 선언
                tempPosition.x = middle;
                lastBlock.localPosition = lastPosition = tempPosition;

                float rubbleHalfScale = deltaX / 2f;
                CreateRubble(
                    new Vector3(isNegativeNum   //중심점 잡는 용도
        ...
        //이전 블럭의 위치가 계속 바뀌고 있기 때문에
        //무빙에서 이걸 불러오도록 함

        return true;
    }

    void CreateRubble(Vector3 pos, Vector3 scale)   //파편 생성
    {
        ...
        go.AddComponent<Rigidbody>();   //떨어져야 하니 중력 적용
    ...
    void GameOverEffect()
    {
        int childCount = this.transform.childCount;     //하위 옵젝 개수
        ...
        for (int i = 1; i < 20; i++)
        {
            ...
            if (go.name.Equals("Rubble"))   //찾은 애가 러블이라면
                continue;
